...
export PATH=/c/mingw/bin:/c/mingw/msys/1.0/bin
...
: ${LIBCDIO_URL:="http://ftp.gnu.org/gnu/libcdio/$LIBCDIO.tar.gz"}
: ${TAGLIB:="taglib-1.6.3"}
...
: ${MYTHDIR:=$PWD}
#: ${MYTHDIR:=/d/buildbot/mythtv/common}
...
branch=$MYTHBRANCH
[ -z "$branch" ] && branch=`gitbranch "$MYTHDIR/mythtv"` && 0
case "$branch" in
    "") : ${MYTHVER:="master"} ;;
